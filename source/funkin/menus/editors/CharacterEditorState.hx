...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		healthBarBG = new FlxSprite(30, FlxG.height - 75).loadGraphic(Paths.image('legacy/healthBar'));
		...
		UI_characterbox.resize(450, 300);
		UI_characterbox.x = UI_box.x - 200;
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSky:BackgroundStageSprite = new BackgroundStageSprite('weeb/weebSky', OFFSET_X - (playerXDifference / 2) - 300, 0 - playerYDifference, 0.1, 0.1);
			bgLayer.add(bgSky);
			bgSky.antialiasing = false;

			var repositionShit = -200 + OFFSET_X - playerXDifference;

			var bgSchool:BackgroundStageSprite = new BackgroundStageSprite('weeb/weebSchool', repositionShit, -playerYDifference + 6, 0.6, 0.90);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BackgroundStageSprite = new BackgroundStageSprite('weeb/weebStreet', repositionShit, -playerYDifference, 0.95, 0.95);
			...
			var bg:BackgroundStageSprite = new BackgroundStageSprite('stageback', -600 + OFFSET_X - playerXDifference, -300, 0.9, 0.9);
			bgLayer.add(bg);

			var stageFront:BackgroundStageSprite = new BackgroundStageSprite('stagefront', -650 + OFFSET_X - playerXDifference, 500, 0.9, 0.9);
	...
	var healthColorStepperA:FlxUINumericStepper;
	...
	function addCharacterUI() {
		...
		var decideIconColor:FlxButton = new FlxButton(reloadImage.x, reloadImage.y + 30, "Get Icon Color", function()
			{
				var coolColor = FlxColor.fromInt(ColorUtil.pickDominantColorOfSprite(leHealthIcon));

				healthColorStepperR.value = coolColor.red;
				healthColorStepperG.value = coolColor.green;
				healthColorStepperB.value = coolColor.blue;
				healthColorStepperA.value = coolColor.alpha;
				...
				getEvent(FlxUINumericStepper.CHANGE_EVENT, healthColorStepperA, null);
			});
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x + 65, noAntialiasingCheckBox.y + 40, "Save Character", function() {
			saveCharacter();
		});
		...
		healthColorStepperA = new FlxUINumericStepper(singDurationStepper.x + 195, saveCharacterButton.y, 20, char.healthColorArray[3], 0, 255, 0);
		...
		tab_group.add(new FlxText(healthColorStepperR.x, healthColorStepperR.y - 18, 0, 'Health bar R/G/B/A:'));
		...
		tab_group.add(healthColorStepperA);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if (sender == healthColorStepperA)
			{
				char.healthColorArray[3] = Math.round(healthColorStepperA.value);
				reloadHealthBarColors();
			}
		}
	}
	...
	{
		healthBarBG.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2], char.healthColorArray[3]);
	}
	...
	function reloadCharacterDropDown() {
		...
		#else
		characterList = FunkinUtil.coolTextFile(Paths.txt('characterList'));
		#end
	...
	function resetHealthBarColor() {
		...
		healthColorStepperA.value = char.healthColorArray[3];
		reloadHealthBarColors();
	}
